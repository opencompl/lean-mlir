...
attribute [bv_multi_width_normalize] BitVec.ofNat_eq_ofNat

/-- Canonicalize multiplications by numerals. -/
@[bv_multi_width_normalize] theorem BitVec.mul_nat_eq_ofNat_mul (x : BitVec w) (n : Nat) :
...
@[bv_multi_width_normalize] theorem BitVec.zero_mul (x : BitVec w) : 0#w * x = 0#w := by simp
...
-/
simproc↓ [bv_multi_width_normalize] shiftLeft_break_down ((BitVec.ofNat _ _) * (_ : BitVec _)) := fun x => do
  match_expr x with
...
attribute [bv_multi_width_normalize] ofBool_1_iff_true ofBool_0_iff_false

-- Not
attribute [bv_multi_width_normalize] Classical.not_not

@[bv_multi_width_normalize] theorem not_eq_eq {α : Sort u} (a b : α) : (¬ (a = b)) = (a ≠ b) := by simp

-- Iff
@[bv_multi_width_normalize] theorem iff_eq (p q : Prop) : (p ↔ q) = (p = q) := by
  by_cases p <;> by_cases q <;> simp [*]

-- Eq
attribute [bv_multi_width_normalize] eq_self heq_eq_eq

-- Prop equality
@[bv_multi_width_normalize] theorem eq_true_eq (p : Prop) : (p = True) = p := by simp
@[bv_multi_width_normalize] theorem eq_false_eq (p : Prop) : (p = False) = ¬p := by simp
@[bv_multi_width_normalize] theorem not_eq_prop (p q : Prop) : (¬(p = q)) = (p = ¬q) := by
  by_cases p <;> by_cases q <;> simp [*]

-- True
attribute [bv_multi_width_normalize] not_true

-- False
attribute [bv_multi_width_normalize] not_false_eq_true
...
@[bv_multi_width_normalize] theorem true_imp_eq (p : Prop) : (True → p) = p := by simp
@[bv_multi_width_normalize] theorem false_imp_eq (p : Prop) : (False → p) = True := by simp
@[bv_multi_width_normalize] theorem imp_true_eq (p : Prop) : (p → True) = True := by simp
@[bv_multi_width_normalize] theorem imp_false_eq (p : Prop) : (p → False) = ¬p := by simp
@[bv_multi_width_normalize] theorem imp_self_eq (p : Prop) : (p → p) = True := by simp

-- And
@[bv_multi_width_normalize↓] theorem not_and (p q : Prop) : (¬(p ∧ q)) = (¬p ∨ ¬q) := by
  by_cases p <;> by_cases q <;> simp [*]
attribute [bv_multi_width_normalize] and_true true_and and_false false_and and_assoc

-- TODO categorize
attribute [bv_multi_width_normalize]
  BitVec.ofBool_or_ofBool ofBool_1_iff_true bne_iff_ne

-- Or
attribute [bv_multi_width_normalize↓] not_or
attribute [bv_multi_width_normalize] or_true true_or or_false false_or or_assoc

-- ite
attribute [bv_multi_width_normalize] ite_true ite_false
@[bv_multi_width_normalize↓] theorem not_ite {_ : Decidable p} (q r : Prop) : (¬ite p q r) = ite p (¬q) (¬r) := by
  by_cases p <;> simp [*]

@[bv_multi_width_normalize] theorem ite_true_false {_ : Decidable p} : (ite p True False) = p := by
  by_cases p <;> simp

@[bv_multi_width_normalize] theorem ite_false_true {_ : Decidable p} : (ite p False True) = ¬p := by
  by_cases p <;> simp
...
@[bv_multi_width_normalize] theorem Bool.or_eq_false : ((a || b) = false) = (a = false ∧ b = false) := by grind

-- Bool and
attribute [bv_multi_width_normalize]
  Bool.and_false Bool.and_true Bool.false_and Bool.true_and Bool.and_eq_true Bool.and_assoc

@[bv_multi_width_normalize] theorem Bool.and_eq_false : ((a && b) = false) = (a = false ∨ b = false) := by grind
...
-- beq
attribute [bv_multi_width_normalize] beq_iff_eq

-- bne
attribute [bv_multi_width_normalize] bne_iff_ne

-- Bool not eq true/false
attribute [bv_multi_width_normalize] Bool.not_eq_false Bool.not_eq_true

-- decide
attribute [bv_multi_width_normalize] decide_eq_true_eq decide_not not_decide_eq_true

-- Nat LE
attribute [bv_multi_width_normalize] Nat.le_zero_eq

-- Nat/Int LT
@[bv_multi_width_normalize] theorem Nat.lt_eq (a b : Nat) : (a < b) = (a + 1 ≤ b) := by
  simp [Nat.lt, LT.lt]

@[bv_multi_width_normalize] theorem Int.lt_eq (a b : Int) : (a < b) = (a + 1 ≤ b) := by
  simp [Int.lt, LT.lt]

-- GT GE
attribute [bv_multi_width_normalize] GT.gt GE.ge

-- Succ
attribute [bv_multi_width_normalize] Nat.succ_eq_add_one
...
#guard_msgs in example : ∀ (a b : BitVec w), a &&& b = 0#w → a = b := by
 bv_multi_width_normalize; trace_state; sorry
