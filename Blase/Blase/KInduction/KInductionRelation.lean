...
def mkCarryAssignCircuitNAux {arity : Type _}
  ...
  [Fintype arity]
  [Hashable arity]
  (p : NFSM arity) (n : Nat) : Circuit (Vars p.α arity (n + 1)) :=
    (p.nextStateCirc).map fun v =>
      match v with
        | .cur t => Vars.stateN t n
        | .input i => Vars.inputN i n
        | .next t => Vars.stateN t (n + 1)
...
theorem mkCarryAssignCircuitNAux_eval_eq_ {arity : Type _}
    ...
    {env' : NFSM.TransitionStateSpace p.α arity → Bool}
    (hEnvStateCur : ∀ (s : p.α), env (Vars.stateN s n) = env' (.cur s))
    (hEnvStateNext : ∀ (s : p.α), env (Vars.stateN s (n + 1)) = env' (.next s))
    (hEnvInput : ∀ (i : arity), env (Vars.inputN i n) = env' (.input i)) :
    ((mkCarryAssignCircuitNAux p n).eval env) = ((p.nextStateCirc).eval env') := by
...
theorem mkCarryAssignCircuitNAux_eval_eq {arity : Type _}
    ...
    (p : NFSM arity) (n : Nat)
    {env : Vars p.α arity (n + 1) → Bool} :
    ((mkCarryAssignCircuitNAux p n).eval env) =
      ((p.nextStateCirc).eval
      (fun x =>
        match x with
        | .cur x => env (Vars.stateN x n)
        | .input x => env (Vars.inputN x n)
        | .next x => env (Vars.stateN x (n + 1)))) := by
  rw [mkCarryAssignCircuitNAux]
  rw [Circuit.eval_map]
  congr
  ext x
...
theorem eval_mkCarryAssignCircuitN_eq_false_iff {arity : Type _}
  ...
  ((mkCarryAssignCircuitN p n).eval env = false) ↔
    ((mkCarryAssignCircuitNAux p n).eval env) := by
...
theorem mkCarryAssignCircuitLeN_eq_false_iff {arity : Type _}
  [DecidableEq arity] [Fintype arity] [Hashable arity] (p : NFSM arity) (n : Nat)
  (env : Vars p.α arity (n + 1) → Bool) :
  ((mkCarryAssignCircuitLeN p n).eval env = false) ↔
  (∀ (i : Nat), (hi : i < n + 1) →
     ((mkCarryAssignCircuitNAux p i).map (fun v => v.castLe (by omega))).eval env) := by
...
theorem eval_mkOutputAssignCircuitNAux_eq {arity : Type _}
  ...
  [Fintype arity]
  [Hashable arity]
  (p : NFSM arity) (n : Nat) (env : Vars p.α arity (n + 1) → Bool) :
...
structure KInductionCircuits {arity : Type _}
  [DecidableEq arity] [Fintype arity] [Hashable arity] (fsm : NFSM arity) (n : Nat) where
...
structure KInductionCircuits.IsLawful {arity : Type _}
    [DecidableEq arity] [Fintype arity] [Hashable arity] {fsm : NFSM arity} {n : Nat}
    ...
    ∀ {env : Vars fsm.α arity (n + 2) → Bool},
      ...
      ↔ (∀ (s : fsm.α) (i : Nat) (hi : i < n + 2),
        env (Vars.stateN s (i + 1)) =
          ((mkCarryAssignCircuitNAux fsm i).map
