...
def cumul_solving_time_smtlib(df, name):
    ...
    sorted2 = np.sort(df['time_cpu_lw'])
    sorted3 = np.sort(df['time_cpu_coq'])
    cumtime1 = [0]
    cumtime1.extend(np.cumsum(sorted1))
    cumtime2 = [0]
    cumtime2.extend(np.cumsum(sorted2))
...
def scatter_solving_time_smtlib(df_sat, df_unsat): 
    ...
    ax[0].set_ylim(10**(np.floor(np.log10(time_min))), 10**(np.floor(np.log10(time_max))))
    ax[1].set_ylim(10**(np.floor(np.log10(time_min))), 10**(np.floor(np.log10(time_max))))
...
def scatter_solving_time_instcombine(df): 
    ...
    time_max = min(df["bitwuzla"].max(), tot_sum.max())
