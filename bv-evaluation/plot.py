...
def bv_decide_smtlib_unsat_stacked_perc(df: pd.DataFrame, bm, type):
    ...
    bottom = np.zeros_like(df_sorted["bv_decide-ld"])
    ax.bar(
        x,
        np.divide(
            df_sorted["bv_decide-ld"]
            + df_sorted["bv_decide-rr"]
            + df_sorted["bv_decide-ac"]
            + df_sorted["bv_decide-af"]
            + df_sorted["bv_decide-ecs"]
            + df_sorted["bv_decide-kc"],
            df_sorted["tot-sum"],
        )
        * 100,
        width,
    ...
    ax.bar(
        x,
        np.divide(df_sorted["bv_decide-lrat"], df_sorted["tot-sum"]) * 100,
        width,
        label="LRAT",
        bottom=bottom,
        color=dark_green,
...
def plot_smtlib():
    ...
    df_sat_stats = df_leanwuzla_46k_stats[
        df_leanwuzla_46k_stats["bv_decide-res"] == "sat"
    ]
    df_unsat_stats = df_leanwuzla_46k_stats[
        df_leanwuzla_46k_stats["bv_decide-res"] == "unsat"
    ]
    ...
    df_sat_stats = df_leanwuzla_trusted_46k_stats[
        df_leanwuzla_trusted_46k_stats["bv_decide-res"] == "sat"
    ]
    df_unsat_stats = df_leanwuzla_trusted_46k_stats[
        df_leanwuzla_trusted_46k_stats["bv_decide-res"] == "unsat"
    ]
    bv_decide_smtlib_sat_stacked_perc(df_sat_stats, "trusted_sat", "i")
    bv_decide_smtlib_unsat_stacked_perc(df_unsat_stats, "trusted_unsat", "i")
