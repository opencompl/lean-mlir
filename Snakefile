...
rule hdel_collect_and_plot:
  ...
  shell:
    "bash -c 'source venv/bin/activate && cd bv-evaluation && python3 collect.py hackersdelight -r {params.nreps}' -j {params.nthreads} && "
    "bash -c 'source venv/bin/activate && cd bv-evaluation && python3 plot.py hackersdelight -r {params.nreps}'"
