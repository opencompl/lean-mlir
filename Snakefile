...
rule hdel_compare_make_lean:
  ...
  output:
    GIT_ROOT / "bv-evaluation/results/HackersDelight/{file}_{width}_{r}.lean",
  shell:
    "cp {input} {output} && "
    "{SED} -i  -e \"s/sorry/bv_compare'/g\" -e \"s/WIDTH/{wildcards.width}/g\" {output} "

rule hdel_compare_make_output:
  input:
    GIT_ROOT / "bv-evaluation/results/HackersDelight/{file}_{width}_{r}.lean",
  output:
    GIT_ROOT / "bv-evaluation/results/HackersDelight/{file}_{width}_{r}.txt"
  resources:
...
rule hdel_collect_and_plot:
  input:
    rules.venv.output,
    expand(GIT_ROOT / "bv-evaluation/results/HackersDelight/{file}_{width}_{r}.txt", 
