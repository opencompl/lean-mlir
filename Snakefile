...
#   input:
#     GIT_ROOT / "{ROOT_DIR}/SSA/Projects/InstCombine/HackersDelight/{file}.lean"
#   output:
#     expand(GIT_ROOT / "{ROOT_DIR}/bv-evaluation/results/HackersDelight/{file}_{width}_{r}.lean", width=config["hdel_bv_widths"], r=config["hdel_nreps"])
#   shell:
#     "cp {input} {output} && "
#     "sed -i {output} -e 's/sorry/bv_compare/g' -e 's/WIDTH/{width}/g' "

# rule hdel_compare_make_output:
#   input:
#     "GIT_ROOT / "{ROOT_DIR}/bv-evaluation/results/HackersDelight/{file}_{width}_{r}.lean
#   output:
#     "GIT_ROOT / "{ROOT_DIR}/bv-evaluation/results/HackersDelight/{file}_{width}_{r}.txt"
#   resources:
...
rule hdel_collect_and_plot:
  input:
    rules.venv.output,
    expand("bv-evaluation/results/HackersDelight/{file}_{width}_{r}.txt", 
  ...
  output:
    tex=GIT_ROOT  / "bv-evaluation/bv_performance-hackersdelight.tex",
  ...
  shell:
    "bash -c 'source venv/bin/activate && cd bv-evaluation && python3 collect.py hackersdelight -r {nreps}' -j {hdel_nthreads}"
    "bash -c 'source venv/bin/activate && cd bv-evaluation && python3 plot.py hackersdelight -r {nreps}'"
