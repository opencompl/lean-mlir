...
def alive_simplifyMulDivRem805 (w : Nat) :
  ...
  rcases w with _|_|w''
  · apply BitVec.eq_nil
  · obtain rfl : x = 1#1 := by bv_omega
    rfl
  · rw [BitVec.ult_toNat, BitVec.toNat_ofNat]
...
def alive_simplifyMulDivRem805' (w : Nat) :
  ...
  · simp only [h, ↓reduceIte]
    by_cases a_0 : a = 0; subst a_0; simp
  ...
  · simp only [h, Bool.false_eq_true, ↓reduceIte]
    have a_ne_zero : a ≠ 0 := ha.1
    have a_ne_one : a ≠ 1 := by
      intro a_one
      subst a_one
      simp only [ofNat_eq_ofNat, Bool.not_eq_true] at h
    ...
    have a_ne_allOnes : a ≠ allOnes w := by
      ...
      rw [Nat.mod_eq_of_lt (a := 3) (by omega), @Nat.mod_eq_of_lt 1 (2^w) (by omega),
