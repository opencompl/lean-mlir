...
open Std (BitVec)
open Ctxt (Var)
namespace  canonicalize-constant-low-bit-mask-and-icmp-slt-to-icmp-sgt
...
def p1_vec_splat_before := [llvmfunc|
  llvm.func @p1_vec_splat(%arg0: vector<2xi8>) -> vector<2xi1> {
    ...
    %2 = llvm.icmp "slt" %1, %arg0 : vector<2xi8>
...
def p3_vec_splat_poison_before := [llvmfunc|
  llvm.func @p3_vec_splat_poison(%arg0: vector<3xi8>) -> vector<3xi1> {
    ...
    %10 = llvm.icmp "slt" %9, %arg0 : vector<3xi8>
...
def p3_vec_nonsplat_poison_before := [llvmfunc|
  llvm.func @p3_vec_nonsplat_poison(%arg0: vector<3xi8>) -> vector<3xi1> {
    ...
    %11 = llvm.icmp "slt" %10, %arg0 : vector<3xi8>
...
def n3_before := [llvmfunc|
  llvm.func @n3(%arg0: vector<2xi8>) -> vector<2xi1> {
...
def n4_before := [llvmfunc|
  llvm.func @n4(%arg0: vector<3xi8>) -> vector<3xi1> {
    %0 = llvm.mlir.constant(-1 : i8) : i8
    %1 = llvm.mlir.poison : i8
...
def p3_vec_nonsplat_poison_combined := [llvmfunc|
  llvm.func @p3_vec_nonsplat_poison(%arg0: vector<3xi8>) -> vector<3xi1> {
    %0 = llvm.mlir.constant(dense<[15, 15, 3]> : vector<3xi8>) : vector<3xi8>
    %1 = llvm.icmp "sgt" %arg0, %0 : vector<3xi8>
    llvm.return %1 : vector<3xi1>
  }]
...
theorem inst_combine_nv   : nv_before  ⊑  nv_combined := by
  unfold nv_before nv_combined
  simp_alive_peephole
  sorry
