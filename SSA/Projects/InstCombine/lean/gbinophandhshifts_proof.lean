...
open Std (BitVec)
theorem shl_and_and_thm (x x_1 : _root_.BitVec 8) : x_1 <<< 4 &&& (x <<< 4 &&& 88#8) = (x &&& x_1) <<< 4 &&& 80#8 := sorry

theorem shl_and_and_fail_thm (x x_1 : _root_.BitVec 8) :
  x_1 <<< 4 &&& (x <<< 5 &&& 88#8) = x_1 <<< 4 &&& (x <<< 5 &&& 64#8) := sorry

theorem shl_add_add_thm (x x_1 : _root_.BitVec 8) : x_1 <<< 2 + x <<< 2 + 48#8 = 48#8 + (x + x_1) <<< 2 := sorry

theorem shl_and_xor_thm (x x_1 : _root_.BitVec 8) : x_1 <<< 1 ^^^ x <<< 1 &&& 20#8 = (x &&& 10#8 ^^^ x_1) <<< 1 := sorry

theorem shl_and_add_thm (x x_1 : _root_.BitVec 8) : x_1 <<< 1 + (x <<< 1 &&& 119#8) = ((x &&& 59#8) + x_1) <<< 1 := sorry

theorem lshr_or_and_thm (x x_1 : _root_.BitVec 8) :
  (x_1 >>> 5 ||| 198#8) &&& x >>> 5 = ((x_1 ||| 192#8) &&& x) >>> 5 := sorry
...
theorem lshr_xor_or_good_mask_thm (x x_1 : _root_.BitVec 8) : x_1 >>> 4 ||| x >>> 4 ^^^ 48#8 = (x ||| x_1) >>> 4 ||| 48#8 := sorry

theorem shl_add_and_thm (x x_1 : _root_.BitVec 8) : x_1 <<< 1 &&& x <<< 1 + 123#8 = (x + 61#8 &&& x_1) <<< 1 := sorry
...
theorem xor_ashr_not_fail_lshr_ashr_thm (x x_1 x_2 : _root_.BitVec 8) :
  (Option.bind (if 8 ≤ x_1.toNat then none else some (x_2 >>> x_1)) fun a =>
    ...
    Option.bind (if 8 ≤ x_1.toNat then none else some (x.sshiftRight x_1.toNat)) fun a =>
      Option.bind (if 8 ≤ x_1.toNat then none else some (x_2 >>> x_1)) fun a_1 => some (a ^^^ a_1 ^^^ 255#8) := sorry
