...
open Std (BitVec)
open Ctxt (Var)
namespace  unsigned-mul-lack-of-overflow-check-via-udiv-of-allones
...
def t2_vec_poison_before := [llvmfunc|
  llvm.func @t2_vec_poison(%arg0: vector<3xi8>, %arg1: vector<3xi8>) -> vector<3xi1> {
    ...
    %10 = llvm.icmp "uge" %9, %arg1 : vector<3xi8>
...
def t2_vec_poison_combined := [llvmfunc|
  llvm.func @t2_vec_poison(%arg0: vector<3xi8>, %arg1: vector<3xi8>) -> vector<3xi1> {
    ...
    %2 = "llvm.intr.umul.with.overflow"(%arg0, %arg1) : (vector<3xi8>, vector<3xi8>) -> !llvm.struct<(vector<3xi8>, vector<3xi1>)>
    %3 = llvm.extractvalue %2[1] : !llvm.struct<(vector<3xi8>, vector<3xi1>)> 
    %4 = llvm.xor %3, %1  : vector<3xi1>
