...
open Std (BitVec)
open Ctxt (Var)
namespace  canonicalize-constant-low-bit-mask-and-icmp-ule-to-icmp-ule
...
def p1_vec_splat_before := [llvmfunc|
  llvm.func @p1_vec_splat() -> vector<2xi1> {
    ...
    %3 = llvm.icmp "ule" %1, %2 : vector<2xi8>
...
def p3_vec_splat_poison_before := [llvmfunc|
  llvm.func @p3_vec_splat_poison() -> vector<3xi1> {
    ...
    %11 = llvm.icmp "ule" %9, %10 : vector<3xi8>
...
def p2_vec_nonsplat_edgecase0_combined := [llvmfunc|
  llvm.func @p2_vec_nonsplat_edgecase0() -> vector<2xi1> {
    ...
    %3 = llvm.call @gen2x8() : () -> vector<2xi8>
...
def p2_vec_nonsplat_edgecase1_combined := [llvmfunc|
  llvm.func @p2_vec_nonsplat_edgecase1() -> vector<2xi1> {
    %0 = llvm.mlir.constant(dense<[3, -1]> : vector<2xi8>) : vector<2xi8>
    %1 = llvm.call @gen2x8() : () -> vector<2xi8>
    %2 = llvm.icmp "ule" %1, %0 : vector<2xi8>
    llvm.return %2 : vector<2xi1>
  }]
...
def p3_vec_splat_poison_combined := [llvmfunc|
  llvm.func @p3_vec_splat_poison() -> vector<3xi1> {
    ...
    %2 = llvm.icmp "ult" %1, %0 : vector<3xi8>
