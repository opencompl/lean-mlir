import SSA.Projects.InstCombine.LLVM.PrettyEDSL
import SSA.Projects.InstCombine.AliveStatements
import SSA.Projects.InstCombine.Refinement
import SSA.Projects.InstCombine.Tactic
open MLIR AST
open Std (BitVec)
open Ctxt (Var)
namespace  or-shifted-masks
set_option pp.proofs false
set_option pp.proofs.withType false
set_option linter.deprecated false
def or_and_shifts1_before := [llvmfunc|
  llvm.func @or_and_shifts1(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(3 : i32) : i32
    %1 = llvm.mlir.constant(15 : i32) : i32
    %2 = llvm.mlir.constant(5 : i32) : i32
    %3 = llvm.mlir.constant(60 : i32) : i32
    %4 = llvm.shl %arg0, %0  : i32
    %5 = llvm.and %4, %1  : i32
    %6 = llvm.shl %arg0, %2  : i32
    %7 = llvm.and %6, %3  : i32
    %8 = llvm.or %5, %7  : i32
    llvm.return %8 : i32
  }]

def or_and_shifts2_before := [llvmfunc|
  llvm.func @or_and_shifts2(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(3 : i32) : i32
    %1 = llvm.mlir.constant(896 : i32) : i32
    %2 = llvm.mlir.constant(4 : i32) : i32
    %3 = llvm.mlir.constant(7 : i32) : i32
    %4 = llvm.shl %arg0, %0  : i32
    %5 = llvm.and %4, %1  : i32
    %6 = llvm.lshr %arg0, %2  : i32
    %7 = llvm.and %6, %3  : i32
    %8 = llvm.or %5, %7  : i32
    llvm.return %8 : i32
  }]

def or_and_shift_shift_and_before := [llvmfunc|
  llvm.func @or_and_shift_shift_and(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(7 : i32) : i32
    %1 = llvm.mlir.constant(3 : i32) : i32
    %2 = llvm.mlir.constant(2 : i32) : i32
    %3 = llvm.mlir.constant(28 : i32) : i32
    %4 = llvm.and %arg0, %0  : i32
    %5 = llvm.shl %4, %1  : i32
    %6 = llvm.shl %arg0, %2  : i32
    %7 = llvm.and %6, %3  : i32
    %8 = llvm.or %5, %7  : i32
    llvm.return %8 : i32
  }]

def multiuse1_before := [llvmfunc|
  llvm.func @multiuse1(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(2 : i32) : i32
    %1 = llvm.mlir.constant(4 : i32) : i32
    %2 = llvm.mlir.constant(6 : i32) : i32
    %3 = llvm.mlir.constant(1 : i32) : i32
    %4 = llvm.and %arg0, %0  : i32
    %5 = llvm.and %arg0, %1  : i32
    %6 = llvm.shl %4, %2 overflow<nsw, nuw>  : i32
    %7 = llvm.lshr %4, %3  : i32
    %8 = llvm.shl %5, %2 overflow<nsw, nuw>  : i32
    %9 = llvm.lshr %5, %3  : i32
    %10 = llvm.or %6, %8  : i32
    %11 = llvm.or %7, %9  : i32
    %12 = llvm.or %11, %10  : i32
    llvm.return %12 : i32
  }]

def multiuse2_before := [llvmfunc|
  llvm.func @multiuse2(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(6 : i32) : i32
    %1 = llvm.mlir.constant(8 : i32) : i32
    %2 = llvm.mlir.constant(1 : i32) : i32
    %3 = llvm.mlir.constant(24 : i32) : i32
    %4 = llvm.mlir.constant(96 : i32) : i32
    %5 = llvm.and %arg0, %0  : i32
    %6 = llvm.shl %5, %1 overflow<nsw, nuw>  : i32
    %7 = llvm.shl %5, %2 overflow<nsw, nuw>  : i32
    %8 = llvm.and %arg0, %3  : i32
    %9 = llvm.shl %8, %1 overflow<nsw, nuw>  : i32
    %10 = llvm.shl %8, %2 overflow<nsw, nuw>  : i32
    %11 = llvm.and %arg0, %4  : i32
    %12 = llvm.shl %11, %1 overflow<nsw, nuw>  : i32
    %13 = llvm.shl %11, %2 overflow<nsw, nuw>  : i32
    %14 = llvm.or %6, %9  : i32
    %15 = llvm.or %12, %14  : i32
    %16 = llvm.or %13, %10  : i32
    %17 = llvm.or %7, %16  : i32
    %18 = llvm.or %15, %17  : i32
    llvm.return %18 : i32
  }]

def multiuse3_before := [llvmfunc|
  llvm.func @multiuse3(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(96 : i32) : i32
    %1 = llvm.mlir.constant(6 : i32) : i32
    %2 = llvm.mlir.constant(1 : i32) : i32
    %3 = llvm.mlir.constant(1920 : i32) : i32
    %4 = llvm.mlir.constant(15 : i32) : i32
    %5 = llvm.and %arg0, %0  : i32
    %6 = llvm.shl %5, %1 overflow<nsw, nuw>  : i32
    %7 = llvm.lshr %5, %2  : i32
    %8 = llvm.shl %arg0, %1  : i32
    %9 = llvm.and %8, %3  : i32
    %10 = llvm.or %6, %9  : i32
    %11 = llvm.lshr %arg0, %2  : i32
    %12 = llvm.and %11, %4  : i32
    %13 = llvm.or %7, %12  : i32
    %14 = llvm.or %13, %10  : i32
    llvm.return %14 : i32
  }]

def shl_mask_before := [llvmfunc|
  llvm.func @shl_mask(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(255 : i32) : i32
    %1 = llvm.mlir.constant(8 : i32) : i32
    %2 = llvm.and %arg0, %0  : i32
    %3 = llvm.shl %2, %1  : i32
    %4 = llvm.or %2, %3  : i32
    llvm.return %4 : i32
  }]

def shl_mask_wrong_shl_const_before := [llvmfunc|
  llvm.func @shl_mask_wrong_shl_const(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(255 : i32) : i32
    %1 = llvm.mlir.constant(7 : i32) : i32
    %2 = llvm.and %arg0, %0  : i32
    %3 = llvm.shl %2, %1  : i32
    %4 = llvm.or %2, %3  : i32
    llvm.return %4 : i32
  }]

def shl_mask_weird_type_before := [llvmfunc|
  llvm.func @shl_mask_weird_type(%arg0: i37) -> i37 {
    %0 = llvm.mlir.constant(255 : i37) : i37
    %1 = llvm.mlir.constant(8 : i37) : i37
    %2 = llvm.and %arg0, %0  : i37
    %3 = llvm.shl %2, %1  : i37
    %4 = llvm.or %2, %3  : i37
    llvm.return %4 : i37
  }]

def shl_mask_extra_use_before := [llvmfunc|
  llvm.func @shl_mask_extra_use(%arg0: i32, %arg1: !llvm.ptr) -> i32 {
    %0 = llvm.mlir.constant(255 : i32) : i32
    %1 = llvm.mlir.constant(8 : i32) : i32
    %2 = llvm.and %arg0, %0  : i32
    %3 = llvm.shl %2, %1  : i32
    llvm.store %3, %arg1 {alignment = 4 : i64} : i32, !llvm.ptr]

    %4 = llvm.or %2, %3  : i32
    llvm.return %4 : i32
  }]

def shl_mul_mask_before := [llvmfunc|
  llvm.func @shl_mul_mask(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(255 : i32) : i32
    %1 = llvm.mlir.constant(65537 : i32) : i32
    %2 = llvm.mlir.constant(8 : i32) : i32
    %3 = llvm.and %arg0, %0  : i32
    %4 = llvm.mul %3, %1  : i32
    %5 = llvm.shl %3, %2  : i32
    %6 = llvm.or %4, %5  : i32
    llvm.return %6 : i32
  }]

def shl_mul_mask_wrong_mul_const_before := [llvmfunc|
  llvm.func @shl_mul_mask_wrong_mul_const(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(255 : i32) : i32
    %1 = llvm.mlir.constant(65535 : i32) : i32
    %2 = llvm.mlir.constant(8 : i32) : i32
    %3 = llvm.and %arg0, %0  : i32
    %4 = llvm.mul %3, %1  : i32
    %5 = llvm.shl %3, %2  : i32
    %6 = llvm.or %4, %5  : i32
    llvm.return %6 : i32
  }]

def or_and_shifts1_combined := [llvmfunc|
  llvm.func @or_and_shifts1(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(3 : i32) : i32
    %1 = llvm.mlir.constant(8 : i32) : i32
    %2 = llvm.mlir.constant(5 : i32) : i32
    %3 = llvm.mlir.constant(32 : i32) : i32
    %4 = llvm.shl %arg0, %0  : i32
    %5 = llvm.and %4, %1  : i32
    %6 = llvm.shl %arg0, %2  : i32
    %7 = llvm.and %6, %3  : i32
    %8 = llvm.or %5, %7  : i32
    llvm.return %8 : i32
  }]

theorem inst_combine_or_and_shifts1   : or_and_shifts1_before  ⊑  or_and_shifts1_combined := by
  unfold or_and_shifts1_before or_and_shifts1_combined
  simp_alive_peephole
  sorry
def or_and_shifts2_combined := [llvmfunc|
  llvm.func @or_and_shifts2(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(3 : i32) : i32
    %1 = llvm.mlir.constant(896 : i32) : i32
    %2 = llvm.mlir.constant(4 : i32) : i32
    %3 = llvm.mlir.constant(7 : i32) : i32
    %4 = llvm.shl %arg0, %0  : i32
    %5 = llvm.and %4, %1  : i32
    %6 = llvm.lshr %arg0, %2  : i32
    %7 = llvm.and %6, %3  : i32
    %8 = llvm.or %5, %7  : i32
    llvm.return %8 : i32
  }]

theorem inst_combine_or_and_shifts2   : or_and_shifts2_before  ⊑  or_and_shifts2_combined := by
  unfold or_and_shifts2_before or_and_shifts2_combined
  simp_alive_peephole
  sorry
def or_and_shift_shift_and_combined := [llvmfunc|
  llvm.func @or_and_shift_shift_and(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(3 : i32) : i32
    %1 = llvm.mlir.constant(56 : i32) : i32
    %2 = llvm.mlir.constant(2 : i32) : i32
    %3 = llvm.mlir.constant(28 : i32) : i32
    %4 = llvm.shl %arg0, %0  : i32
    %5 = llvm.and %4, %1  : i32
    %6 = llvm.shl %arg0, %2  : i32
    %7 = llvm.and %6, %3  : i32
    %8 = llvm.or %5, %7  : i32
    llvm.return %8 : i32
  }]

theorem inst_combine_or_and_shift_shift_and   : or_and_shift_shift_and_before  ⊑  or_and_shift_shift_and_combined := by
  unfold or_and_shift_shift_and_before or_and_shift_shift_and_combined
  simp_alive_peephole
  sorry
def multiuse1_combined := [llvmfunc|
  llvm.func @multiuse1(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(1 : i32) : i32
    %1 = llvm.mlir.constant(2 : i32) : i32
    %2 = llvm.mlir.constant(6 : i32) : i32
    %3 = llvm.mlir.constant(384 : i32) : i32
    %4 = llvm.lshr %arg0, %0  : i32
    %5 = llvm.and %4, %0  : i32
    %6 = llvm.lshr %arg0, %0  : i32
    %7 = llvm.and %6, %1  : i32
    %8 = llvm.shl %arg0, %2  : i32
    %9 = llvm.and %8, %3  : i32
    %10 = llvm.or %5, %7  : i32
    %11 = llvm.or %10, %9  : i32
    llvm.return %11 : i32
  }]

theorem inst_combine_multiuse1   : multiuse1_before  ⊑  multiuse1_combined := by
  unfold multiuse1_before multiuse1_combined
  simp_alive_peephole
  sorry
def multiuse2_combined := [llvmfunc|
  llvm.func @multiuse2(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(1 : i32) : i32
    %1 = llvm.mlir.constant(12 : i32) : i32
    %2 = llvm.mlir.constant(48 : i32) : i32
    %3 = llvm.mlir.constant(192 : i32) : i32
    %4 = llvm.mlir.constant(8 : i32) : i32
    %5 = llvm.mlir.constant(32256 : i32) : i32
    %6 = llvm.shl %arg0, %0  : i32
    %7 = llvm.and %6, %1  : i32
    %8 = llvm.shl %arg0, %0  : i32
    %9 = llvm.and %8, %2  : i32
    %10 = llvm.shl %arg0, %0  : i32
    %11 = llvm.and %10, %3  : i32
    %12 = llvm.shl %arg0, %4  : i32
    %13 = llvm.and %12, %5  : i32
    %14 = llvm.or %11, %9  : i32
    %15 = llvm.or %7, %14  : i32
    %16 = llvm.or %13, %15  : i32
    llvm.return %16 : i32
  }]

theorem inst_combine_multiuse2   : multiuse2_before  ⊑  multiuse2_combined := by
  unfold multiuse2_before multiuse2_combined
  simp_alive_peephole
  sorry
def multiuse3_combined := [llvmfunc|
  llvm.func @multiuse3(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(1 : i32) : i32
    %1 = llvm.mlir.constant(48 : i32) : i32
    %2 = llvm.mlir.constant(6 : i32) : i32
    %3 = llvm.mlir.constant(8064 : i32) : i32
    %4 = llvm.mlir.constant(15 : i32) : i32
    %5 = llvm.lshr %arg0, %0  : i32
    %6 = llvm.and %5, %1  : i32
    %7 = llvm.shl %arg0, %2  : i32
    %8 = llvm.and %7, %3  : i32
    %9 = llvm.lshr %arg0, %0  : i32
    %10 = llvm.and %9, %4  : i32
    %11 = llvm.or %6, %10  : i32
    %12 = llvm.or %11, %8  : i32
    llvm.return %12 : i32
  }]

theorem inst_combine_multiuse3   : multiuse3_before  ⊑  multiuse3_combined := by
  unfold multiuse3_before multiuse3_combined
  simp_alive_peephole
  sorry
def shl_mask_combined := [llvmfunc|
  llvm.func @shl_mask(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(255 : i32) : i32
    %1 = llvm.mlir.constant(8 : i32) : i32
    %2 = llvm.and %arg0, %0  : i32
    %3 = llvm.shl %2, %1 overflow<nsw, nuw>  : i32
    %4 = llvm.or %2, %3  : i32
    llvm.return %4 : i32
  }]

theorem inst_combine_shl_mask   : shl_mask_before  ⊑  shl_mask_combined := by
  unfold shl_mask_before shl_mask_combined
  simp_alive_peephole
  sorry
def shl_mask_wrong_shl_const_combined := [llvmfunc|
  llvm.func @shl_mask_wrong_shl_const(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(255 : i32) : i32
    %1 = llvm.mlir.constant(7 : i32) : i32
    %2 = llvm.and %arg0, %0  : i32
    %3 = llvm.shl %2, %1 overflow<nsw, nuw>  : i32
    %4 = llvm.or %2, %3  : i32
    llvm.return %4 : i32
  }]

theorem inst_combine_shl_mask_wrong_shl_const   : shl_mask_wrong_shl_const_before  ⊑  shl_mask_wrong_shl_const_combined := by
  unfold shl_mask_wrong_shl_const_before shl_mask_wrong_shl_const_combined
  simp_alive_peephole
  sorry
def shl_mask_weird_type_combined := [llvmfunc|
  llvm.func @shl_mask_weird_type(%arg0: i37) -> i37 {
    %0 = llvm.mlir.constant(255 : i37) : i37
    %1 = llvm.mlir.constant(8 : i37) : i37
    %2 = llvm.and %arg0, %0  : i37
    %3 = llvm.shl %2, %1 overflow<nsw, nuw>  : i37
    %4 = llvm.or %2, %3  : i37
    llvm.return %4 : i37
  }]

theorem inst_combine_shl_mask_weird_type   : shl_mask_weird_type_before  ⊑  shl_mask_weird_type_combined := by
  unfold shl_mask_weird_type_before shl_mask_weird_type_combined
  simp_alive_peephole
  sorry
def shl_mask_extra_use_combined := [llvmfunc|
  llvm.func @shl_mask_extra_use(%arg0: i32, %arg1: !llvm.ptr) -> i32 {
    %0 = llvm.mlir.constant(255 : i32) : i32
    %1 = llvm.mlir.constant(8 : i32) : i32
    %2 = llvm.and %arg0, %0  : i32
    %3 = llvm.shl %2, %1 overflow<nsw, nuw>  : i32
    llvm.store %3, %arg1 {alignment = 4 : i64} : i32, !llvm.ptr]

theorem inst_combine_shl_mask_extra_use   : shl_mask_extra_use_before  ⊑  shl_mask_extra_use_combined := by
  unfold shl_mask_extra_use_before shl_mask_extra_use_combined
  simp_alive_peephole
  sorry
    %4 = llvm.or %2, %3  : i32
    llvm.return %4 : i32
  }]

theorem inst_combine_shl_mask_extra_use   : shl_mask_extra_use_before  ⊑  shl_mask_extra_use_combined := by
  unfold shl_mask_extra_use_before shl_mask_extra_use_combined
  simp_alive_peephole
  sorry
def shl_mul_mask_combined := [llvmfunc|
  llvm.func @shl_mul_mask(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(255 : i32) : i32
    %1 = llvm.mlir.constant(65537 : i32) : i32
    %2 = llvm.mlir.constant(8 : i32) : i32
    %3 = llvm.and %arg0, %0  : i32
    %4 = llvm.mul %3, %1 overflow<nsw, nuw>  : i32
    %5 = llvm.shl %3, %2 overflow<nsw, nuw>  : i32
    %6 = llvm.or %4, %5  : i32
    llvm.return %6 : i32
  }]

theorem inst_combine_shl_mul_mask   : shl_mul_mask_before  ⊑  shl_mul_mask_combined := by
  unfold shl_mul_mask_before shl_mul_mask_combined
  simp_alive_peephole
  sorry
def shl_mul_mask_wrong_mul_const_combined := [llvmfunc|
  llvm.func @shl_mul_mask_wrong_mul_const(%arg0: i32) -> i32 {
    %0 = llvm.mlir.constant(255 : i32) : i32
    %1 = llvm.mlir.constant(65535 : i32) : i32
    %2 = llvm.mlir.constant(8 : i32) : i32
    %3 = llvm.and %arg0, %0  : i32
    %4 = llvm.mul %3, %1 overflow<nsw, nuw>  : i32
    %5 = llvm.shl %3, %2 overflow<nsw, nuw>  : i32
    %6 = llvm.or %4, %5  : i32
    llvm.return %6 : i32
  }]

theorem inst_combine_shl_mul_mask_wrong_mul_const   : shl_mul_mask_wrong_mul_const_before  ⊑  shl_mul_mask_wrong_mul_const_combined := by
  unfold shl_mul_mask_wrong_mul_const_before shl_mul_mask_wrong_mul_const_combined
  simp_alive_peephole
  sorry
