...
# }
def allowed(op):
        return hasattr(op, "sym_name") or (op.name== "builtin.unregistered" and op.op_name.data in allowed_unregistered)  or (op.name in allowed_names)
def show(block):
        output = io.StringIO()
        ...
        with open(os.path.join("../vcombined-mlir", filename), "r") as f1:
                with open(os.path.join("../vbefore-mlir", filename), "r") as f2:
                        ...
                        try:
                                ...
                                for func in funcs:
                                        try:
                                                other = next(f for f in module2.walk() if isinstance(f, FuncOp) and f.sym_name == func.sym_name)
