...
def get_lines(msg):
    ...
    pattern = re.compile(r"info: .+?:(\d+):\d+: (theorem \S+\.extracted_\S+ .+?)(?=\nwarning:)", flags=re.DOTALL)
...
def process_file(file_path):
    ...
    proof_file = file_path[:-5].replace("/LLVM/", "/generalizer/") + "_proof.lean"
    stem_name = file_path.split("/")[-1][:-5]
    new_file_path = file_path.replace("/LLVM/", "/generalizer/")
    log_path = proof_file.replace("/generalizer/", "/logs/generalizer/").replace(".lean", ".txt")
...
def main():
    rm_proofs = "\nrm -r " + generalizer_proof_path + "/*\n"
    subprocess.run(rm_proofs, shell=True)
    
    rm_logs = "\nrm -r " + generalizer_log_path + "/*\n"
