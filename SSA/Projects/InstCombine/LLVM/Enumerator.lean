...
def generateRawSemantics : IO Unit := do
  ...
  rows := rows.append (binopRows "or"   (fun w a b => InstCombine.Op.denote (.or false w)   [a,b]ₕ))
  ...
  rows := rows.append (binopRows "sub"  (fun w a b => InstCombine.Op.denote (.sub false false w)  [a,b]ₕ))
  rows := rows.append (binopRows "mul"  (fun w a b => InstCombine.Op.denote (.mul false false w)  [a,b]ₕ))
  rows := rows.append (binopRows "udiv" (fun w a b => InstCombine.Op.denote (.udiv false w) [a,b]ₕ))
  rows := rows.append (binopRows "sdiv" (fun w a b => InstCombine.Op.denote (.sdiv false w) [a,b]ₕ))
  ...
  rows := rows.append (binopRows "shl"  (fun w a b => InstCombine.Op.denote (.shl false false w)  [a,b]ₕ))
  rows := rows.append (binopRows "lshr" (fun w a b => InstCombine.Op.denote (.lshr false w) [a,b]ₕ))
  rows := rows.append (binopRows "ashr" (fun w a b => InstCombine.Op.denote (.ashr false w) [a,b]ₕ))
