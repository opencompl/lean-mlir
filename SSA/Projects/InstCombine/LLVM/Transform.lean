...
abbrev Expr (Γ : Context φ) (ty : MTy φ)  := Expr (MOp φ) Γ ty
abbrev Com (Γ : Context φ) (ty : MTy φ)   := Com (MOp φ) Γ ty
...
def mkComInstantiate (reg : Region φ) :
    ExceptM (Vector Nat φ → Σ (Γ : Ctxt Ty) (ty : Ty), Com InstCombine.Op Γ ty) := do
