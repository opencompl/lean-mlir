...
@[simp]
theorem Bitvec.ofNat_zero : Bitvec.ofNat w 0 = 0 := by
  induction w
  case zero => simp[Bitvec.ofNat]
  case succ w' wh =>
    simp[Bitvec.ofNat]
    rw[wh]
...
@[simp]
theorem Bitvec.ofNat_one : Bitvec.ofNat w 1 = 1 := by
  cases w
  case zero => simp[Bitvec.ofNat]
  case succ w' =>
    simp[Bitvec.ofNat]
...
theorem AddSub_1043_refinement (w : Nat) : AddSub_1043_src w ⊑ AddSub_1043_tgt w := by
  ...
  /- simplification -/
  simp_mlir
  simp[Bitvec.ofInt, Bitvec.ofNat]
  intros x0 x1 x2
  -- | this does not work due to the mismatch between Mathlib.Bitvec and Std.bitvec
  -- apply bitvec_AddSub_1043 (w := w) (Z := x1)
