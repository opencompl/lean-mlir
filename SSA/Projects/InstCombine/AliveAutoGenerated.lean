...
theorem LLVM.shl?_eq_some_iff :
    LLVM.shl? x? y? = some z ↔ _ := by
  sorry
...
theorem alive_AddSub_1156  (w : Nat)   : alive_AddSub_1156_src w  ⊑ alive_AddSub_1156_tgt w  := by
  ...
  simp only [LLVM.add?, LLVM.const?, pure_bind]
  intro a
  -- rw [BitVec.Refinement.bind_same]
  cases' a with a
  · simp only [Option.none_bind'', BitVec.Refinement.refl]
  · simp
