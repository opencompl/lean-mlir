...
theorem pv_proof.pv_thm_1 (e e_1 : IntW 8) :
  icmp IntPred.eq (LLVM.and (lshr (const? 8 (-1)) e_1) e) e ⊑ icmp IntPred.ule e (lshr (const? 8 (-1)) e_1) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
...
theorem n0_proof.n0_thm_1 (e : IntW 8) :
  icmp IntPred.eq (LLVM.and e (const? 8 4)) e ⊑ icmp IntPred.eq (LLVM.and e (const? 8 (-5))) (const? 8 0) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
