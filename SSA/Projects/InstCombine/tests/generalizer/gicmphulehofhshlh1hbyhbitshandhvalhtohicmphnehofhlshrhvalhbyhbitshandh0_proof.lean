...
theorem both_proof.both_thm_1 (e e_1 : IntW 8) :
  icmp IntPred.ule (shl (const? 8 1) e) (shl (const? 8 1) e_1) ⊑
    icmp IntPred.ne (lshr (shl (const? 8 1) e_1 { «nuw» := true }) e) (const? 8 0) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
...
theorem n2_proof.n2_thm_1 (e e_1 : IntW 8) :
  icmp IntPred.ult (shl (const? 8 1) e_1) e ⊑ icmp IntPred.ult (shl (const? 8 1) e_1 { «nuw» := true }) e := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
