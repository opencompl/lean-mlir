
import SSA.Projects.InstCombine.TacticAuto
import SSA.Projects.InstCombine.LLVM.Semantics
import SSA.Experimental.Bits.Fast.Generalize
open BitVec
open LLVM

set_option linter.unusedTactic false
set_option linter.unreachableTactic false
set_option maxHeartbeats 5000000
set_option maxRecDepth 1000000

section gcomparehsigns_proof
theorem test1_proof.test1_thm_1 (e e_1 : IntW 32) :
  zext 32 (LLVM.xor (icmp IntPred.slt e_1 (const? 32 0)) (icmp IntPred.sgt e (const? 32 (-1)))) ⊑
    zext 32 (icmp IntPred.sgt (LLVM.xor e_1 e) (const? 32 (-1))) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
        bv_generalize
        simp_alive_benchmark
        all_goals sorry

    
theorem test2_proof.test2_thm_1 (e e_1 : IntW 32) :
  zext 32 (icmp IntPred.eq (LLVM.and e (const? 32 8)) (LLVM.and e_1 (const? 32 8))) ⊑
    LLVM.xor (LLVM.and (lshr (LLVM.xor e e_1) (const? 32 3)) (const? 32 1)) (const? 32 1) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
        bv_generalize
        simp_alive_benchmark
        all_goals sorry

    
theorem test3_proof.test3_thm_1 (e e_1 : IntW 32) :
  zext 32 (icmp IntPred.eq (lshr e (const? 32 31)) (lshr e_1 (const? 32 31))) ⊑
    zext 32 (icmp IntPred.sgt (LLVM.xor e e_1) (const? 32 (-1))) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
        bv_generalize
        simp_alive_benchmark
        all_goals sorry

    
theorem test3i_proof.test3i_thm_1 (e e_1 : IntW 32) :
  zext 32
      (icmp IntPred.eq (LLVM.or (lshr e (const? 32 29)) (const? 32 35))
        (LLVM.or (lshr e_1 (const? 32 29)) (const? 32 35))) ⊑
    zext 32 (icmp IntPred.sgt (LLVM.xor e e_1) (const? 32 (-1))) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
        bv_generalize
        simp_alive_benchmark
        all_goals sorry

    
theorem test4a_proof.test4a_thm_1 (e : IntW 32) :
  icmp IntPred.slt (LLVM.or (ashr e (const? 32 31)) (lshr (sub (const? 32 0) e) (const? 32 31))) (const? 32 1) ⊑
    icmp IntPred.slt e (const? 32 1) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
        bv_generalize
        simp_alive_benchmark
        all_goals sorry

    
theorem test4b_proof.test4b_thm_1 (e : IntW 64) :
  icmp IntPred.slt (LLVM.or (ashr e (const? 64 63)) (lshr (sub (const? 64 0) e) (const? 64 63))) (const? 64 1) ⊑
    icmp IntPred.slt e (const? 64 1) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
        bv_generalize
        simp_alive_benchmark
        all_goals sorry

    
theorem test4c_proof.test4c_thm_1 (e : IntW 64) :
  icmp IntPred.slt (trunc 32 (LLVM.or (ashr e (const? 64 63)) (lshr (sub (const? 64 0) e) (const? 64 63))))
      (const? 32 1) ⊑
    icmp IntPred.slt e (const? 64 1) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
        bv_generalize
        simp_alive_benchmark
        all_goals sorry

    
theorem shift_trunc_signbit_test_proof.shift_trunc_signbit_test_thm_1 (e : IntW 32) :
  icmp IntPred.slt (trunc 8 (lshr e (const? 32 24))) (const? 8 0) ⊑ icmp IntPred.slt e (const? 32 0) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
        bv_generalize
        simp_alive_benchmark
        all_goals sorry

    
theorem shift_trunc_wrong_shift_proof.shift_trunc_wrong_shift_thm_1 (e : IntW 32) :
  icmp IntPred.slt (trunc 8 (lshr e (const? 32 23))) (const? 8 0) ⊑
    icmp IntPred.ne (LLVM.and e (const? 32 1073741824)) (const? 32 0) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
        bv_generalize
        simp_alive_benchmark
        all_goals sorry

    
theorem shift_trunc_wrong_cmp_proof.shift_trunc_wrong_cmp_thm_1 (e : IntW 32) :
  icmp IntPred.slt (trunc 8 (lshr e (const? 32 24))) (const? 8 1) ⊑
    icmp IntPred.slt (trunc 8 (lshr e (const? 32 24)) { «nuw» := true }) (const? 8 1) := by
        simp_alive_undef
        simp_alive_ops
        simp_alive_case_bash
        simp_alive_split
        bv_generalize
        simp_alive_benchmark
        all_goals sorry

    
