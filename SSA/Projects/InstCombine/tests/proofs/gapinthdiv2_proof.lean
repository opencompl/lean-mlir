...
theorem test1_thm (x : BitVec 333) :
  some (x / 70368744177664#333) ⊑ if 333 % 2 ^ 333 ≤ 46 % 2 ^ 333 then none else some (x >>> (46 % 2 ^ 333)) := sorry
...
theorem test2_thm (x : BitVec 499) :
  (Option.bind (if 499 % 2 ^ 499 ≤ 197 % 2 ^ 499 then none else some (4096#499 <<< (197 % 2 ^ 499))) fun y' =>
    ...
    if 499 % 2 ^ 499 ≤ 209 % 2 ^ 499 then none else some (x >>> (209 % 2 ^ 499)) := sorry
...
theorem test3_thm (x : BitVec 1) (x_1 : BitVec 599) :
  (Option.bind
    ...
    Option.bind
      ...
      fun y' => if 599#599 ≤ y' then none else some (x_1 >>> y'.toNat) := sorry
