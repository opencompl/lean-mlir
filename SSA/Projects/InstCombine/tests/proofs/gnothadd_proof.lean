...
theorem basic_thm (x x_1 : BitVec 8) : (x_1 ^^^ 255#8) + x ^^^ 255#8 = x_1 - x := by bv_compare'

theorem basic_com_add_thm (x x_1 : BitVec 8) : x_1 + (x ^^^ 255#8) ^^^ 255#8 = x - x_1 := by bv_compare'
...
theorem basic_preserve_nuw_thm (x x_1 : BitVec 8) :
  ((if (x_1 ^^^ 255#8) + x < x_1 ^^^ 255#8 ∨ (x_1 ^^^ 255#8) + x < x then none else some ((x_1 ^^^ 255#8) + x)).bind
      fun x' => some (x' ^^^ 255#8)) ⊑
    if x_1 < x then none else some (x_1 - x) := by bv_compare'
