...
theorem t2_thm (e : IntW 32) (e_1 : IntW 64) :
  LLVM.udiv e_1 (zext 64 (shl (const? 1) e)) ⊑ lshr e_1 (zext 64 e) := by 
    simp_alive_undef
    simp_alive_ops
    simp_alive_case_bash
    try alive_auto
...
theorem t7_thm (e : IntW 32) : LLVM.sdiv (shl e (const? 2) { «nsw» := true, «nuw» := false }) e ⊑ const? 4 := by 
    simp_alive_undef
    simp_alive_ops
    simp_alive_case_bash
    try alive_auto
...
theorem t12_thm (e : IntW 32) : LLVM.udiv (shl e (const? 2) { «nsw» := false, «nuw» := true }) e ⊑ const? 4 := by 
    simp_alive_undef
    simp_alive_ops
    simp_alive_case_bash
    try alive_auto
...
theorem sdiv_shl_pair_const_thm (e : IntW 32) :
  LLVM.sdiv (shl e (const? 2) { «nsw» := true, «nuw» := false }) (shl e (const? 1) { «nsw» := true, «nuw» := false }) ⊑
    const? 2 := by 
    simp_alive_undef
    simp_alive_ops
    simp_alive_case_bash
...
theorem udiv_shl_pair_const_thm (e : IntW 32) :
  LLVM.udiv (shl e (const? 2) { «nsw» := false, «nuw» := true }) (shl e (const? 1) { «nsw» := false, «nuw» := true }) ⊑
    const? 2 := by 
    simp_alive_undef
    simp_alive_ops
    simp_alive_case_bash
