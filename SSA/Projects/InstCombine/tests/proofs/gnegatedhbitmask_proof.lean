...
theorem sub_mask1_trunc_lshr_thm (x : BitVec 64) :
  some (10#8 - (setWidth 8 (x >>> 15) &&& 1#8)) ⊑
    if
      ...
      none
    else some (setWidth 8 ((x <<< 48).sshiftRight 63) + 10#8) := by bv_compare'
...
theorem sub_sext_mask1_trunc_lshr_thm (x : BitVec 64) :
  some (10#32 - signExtend 32 (setWidth 8 (x >>> 15) &&& 1#8)) ⊑
    (if
      ...
      fun x' => some (setWidth 32 x') := by bv_compare'
...
theorem sub_zext_trunc_lshr_thm (x : BitVec 64) :
  some (10#32 - setWidth 32 (setWidth 1 (x >>> 15))) ⊑
    if
      ...
      none
    else some ((setWidth 32 x <<< 16).sshiftRight 31 + 10#32) := by bv_compare'
...
theorem neg_signbit_thm (x : BitVec 8) : -setWidth 32 (x >>> 7) = signExtend 32 (x.sshiftRight 7) := by bv_compare'
