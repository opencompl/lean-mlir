...
theorem test6_thm (x : BitVec 55) : x <<< 1 * 3#55 = x * 6#55 := by bv_compare'

theorem test6a_thm (x : BitVec 55) : (x * 3#55) <<< 1 = x * 6#55 := by bv_compare'

theorem test7_thm (x : BitVec 8) :
  (if 29#29 ≤ setWidth 29 x then none else some ((536870911#29).sshiftRight (x.toNat % 536870912))) ⊑
    some 536870911#29 := by bv_compare'
...
theorem test15_thm (x : BitVec 1) :
  (Option.bind
    ...
    | some { toFin := ⟨0, ⋯⟩ } => some 4#45 := by bv_compare'
...
theorem test15a_thm (x : BitVec 1) :
  (Option.bind
    ...
    | some { toFin := ⟨0, ⋯⟩ } => some 128#53 := by bv_compare'

theorem test23_thm (x : BitVec 44) : setWidth 11 ((x <<< 33).sshiftRight 33) = setWidth 11 x := by bv_compare'
