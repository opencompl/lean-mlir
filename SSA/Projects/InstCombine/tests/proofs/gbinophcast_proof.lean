...
theorem and_add_bool_to_select_thm :
  ∀ (e : IntW 32) (e_1 : IntW 1), LLVM.and (add (const? (-1)) (zext 32 e_1)) e ⊑ select e_1 (const? 0) e := by 
    simp_alive_undef
    simp_alive_ops
    simp_alive_case_bash
    try alive_auto
...
theorem and_add_bool_to_select_multi_use_thm :
