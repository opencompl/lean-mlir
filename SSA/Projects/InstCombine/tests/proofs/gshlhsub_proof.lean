...
theorem shl_sub_i64_thm (e : IntW 64) : shl (const? 1) (sub (const? 63) e) ⊑ lshr (const? (-9223372036854775808)) e := by 
    simp_alive_undef
    simp_alive_ops
    simp_alive_case_bash
    try alive_auto
...
theorem shl_bad_sub2_i32_thm (e : IntW 32) :
  shl (const? 1) (sub e (const? 31)) ⊑ shl (const? 1) (add e (const? (-31))) { «nsw» := false, «nuw» := true } := by 
    simp_alive_undef
    simp_alive_ops
    simp_alive_case_bash
    try alive_auto
