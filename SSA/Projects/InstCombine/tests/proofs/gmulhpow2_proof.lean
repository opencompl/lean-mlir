...
theorem mul_selectp2_x_thm (x : BitVec 8) (x_1 : BitVec 1) :
  (Option.bind
    ...
    Option.bind
      ...
      fun y' => if 8#8 ≤ y' then none else some (x <<< y'.toNat) := by bv_compare'
...
theorem mul_selectp2_x_propegate_nuw_thm (x : BitVec 8) (x_1 : BitVec 1) :
  (Option.bind
    ...
    Option.bind
      ...
      | some { toFin := ⟨0, ⋯⟩ } => some 2#8)
      fun y' =>
      if x <<< y'.toNat >>> y'.toNat = x then none else if 8#8 ≤ y' then none else some (x <<< y'.toNat) := by bv_compare'
...
theorem mul_x_selectp2_thm (x : BitVec 1) (x_1 : BitVec 8) :
  (Option.bind
    ...
    Option.bind
      ...
      fun y' => if 8#8 ≤ y' then none else some ((x_1 * x_1) <<< y'.toNat) := by bv_compare'
...
theorem shl_add_log_may_cause_poison_pr62175_with_nuw_thm (x x_1 : BitVec 8) :
  ((if 4#8 <<< x.toNat >>> x.toNat = 4#8 then none else if 8#8 ≤ x then none else some (4#8 <<< x.toNat)).bind fun y' =>
      some (x_1 * y')) ⊑
    if 8#8 ≤ x + 2#8 then none else some (x_1 <<< ((x.toNat + 2) % 256)) := by bv_compare'
