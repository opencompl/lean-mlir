...
theorem mul_selectp2_x_propegate_nuw_thm :
  ∀ (e : IntW 8) (e_1 : IntW 1),
    mul (select e_1 (const? 2) (const? 4)) e { «nsw» := true, «nuw» := true } ⊑
      shl e (select e_1 (const? 1) (const? 2)) { «nsw» := false, «nuw» := true } := by 
    simp_alive_undef
    simp_alive_ops
    simp_alive_case_bash
    try alive_auto
...
theorem shl_add_log_may_cause_poison_pr62175_with_nuw_thm :
  ∀ (e e_1 : IntW 8), mul e_1 (shl (const? 4) e { «nsw» := false, «nuw» := true }) ⊑ shl e_1 (add e (const? 2)) := by 
    simp_alive_undef
    simp_alive_ops
    simp_alive_case_bash
    try alive_auto
...
theorem shl_add_log_may_cause_poison_pr62175_with_nsw_thm :
  ∀ (e e_1 : IntW 8), mul e_1 (shl (const? 4) e { «nsw» := true, «nuw» := false }) ⊑ shl e_1 (add e (const? 2)) := by 
    simp_alive_undef
    simp_alive_ops
    simp_alive_case_bash
    try alive_auto
