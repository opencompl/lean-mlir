...
theorem shl_and_and_thm (x x_1 : BitVec 8) : x_1 <<< 4 &&& (x <<< 4 &&& 88#8) = (x &&& x_1) <<< 4 &&& 80#8 := by bv_compare'

theorem shl_and_and_fail_thm (x x_1 : BitVec 8) : x_1 <<< 4 &&& (x <<< 5 &&& 88#8) = x_1 <<< 4 &&& (x <<< 5 &&& 64#8) := by bv_compare'

theorem shl_add_add_thm (x x_1 : BitVec 8) : x_1 <<< 2 + (x <<< 2 + 48#8) = x <<< 2 + x_1 <<< 2 + 48#8 := by bv_compare'
...
theorem shl_add_add_fail_thm (x x_1 : BitVec 8) :
  some (x_1 >>> 2 + (x >>> 2 + 48#8)) ⊑
    (if (48#8).msb = false ∧ (x >>> 2 + 48#8).msb = true then none
      ...
      fun y' => if x_1 >>> 2 + y' < x_1 >>> 2 ∨ x_1 >>> 2 + y' < y' then none else some (x_1 >>> 2 + y') := by bv_compare'

theorem shl_and_xor_thm (x x_1 : BitVec 8) : x_1 <<< 1 ^^^ x <<< 1 &&& 20#8 = (x_1 ^^^ x &&& 10#8) <<< 1 := by bv_compare'

theorem shl_and_add_thm (x : BitVec 8) : x <<< 1 &&& 119#8 = (x &&& 59#8) <<< 1 := by bv_compare'

theorem lshr_or_and_thm (x x_1 : BitVec 8) : (x_1 >>> 5 ||| 198#8) &&& x >>> 5 = (x &&& (x_1 ||| 192#8)) >>> 5 := by bv_compare'

theorem lshr_or_or_fail_thm (x x_1 : BitVec 8) : x_1 >>> 5 ||| (x >>> 5 ||| 198#8) = (x ||| x_1) >>> 5 ||| 198#8 := by bv_compare'
...
theorem lshr_or_or_no_const_thm (x x_1 x_2 x_3 : BitVec 8) :
  (Option.bind (if 8#8 ≤ x_2 then none else some (x_3 >>> x_2.toNat)) fun a =>
    ...
    Option.bind (if 8#8 ≤ x_2 then none else some ((x_1 ||| x_3) >>> x_2.toNat)) fun a => some (a ||| x) := by bv_compare'
...
theorem lshr_xor_or_good_mask_thm (x x_1 : BitVec 8) : x_1 >>> 4 ||| x >>> 4 ^^^ 48#8 = (x ||| x_1) >>> 4 ||| 48#8 := by bv_compare'

theorem shl_xor_xor_good_mask_thm (x x_1 : BitVec 8) : x_1 <<< 1 ^^^ (x <<< 1 ^^^ 88#8) = (x ^^^ x_1) <<< 1 ^^^ 88#8 := by bv_compare'

theorem shl_xor_xor_bad_mask_distribute_thm (x x_1 : BitVec 8) : x_1 <<< 1 ^^^ (x <<< 1 ^^^ 188#8) = (x ^^^ x_1) <<< 1 ^^^ 188#8 := by bv_compare'

theorem shl_add_and_thm (x x_1 : BitVec 8) : x_1 <<< 1 &&& x <<< 1 + 123#8 = (x_1 &&& x + 61#8) <<< 1 := by bv_compare'
...
theorem and_ashr_not_thm (x x_1 x_2 : BitVec 8) :
  (Option.bind (if 8#8 ≤ x_1 then none else some (x_2.sshiftRight x_1.toNat)) fun a =>
    ...
    if 8#8 ≤ x_1 then none else some ((x_2 &&& (x ^^^ 255#8)).sshiftRight x_1.toNat) := by bv_compare'
...
theorem and_ashr_not_commuted_thm (x x_1 x_2 : BitVec 8) :
  (Option.bind (if 8#8 ≤ x_1 then none else some (x_2.sshiftRight x_1.toNat)) fun a =>
    ...
    if 8#8 ≤ x_1 then none else some ((x &&& (x_2 ^^^ 255#8)).sshiftRight x_1.toNat) := by bv_compare'
...
theorem xor_ashr_not_fail_lshr_ashr_thm (x x_1 x_2 : BitVec 8) :
  (Option.bind (if 8#8 ≤ x_1 then none else some (x_2 >>> x_1.toNat)) fun a =>
    ...
    Option.bind (if 8#8 ≤ x_1 then none else some (x.sshiftRight x_1.toNat)) fun a =>
      Option.bind (if 8#8 ≤ x_1 then none else some (x_2 >>> x_1.toNat)) fun x => some (a ^^^ x ^^^ 255#8) := by bv_compare'
