...
theorem mul_splat_fold_thm (x : _root_.BitVec 32) : 
    (x * 65537#32) >>> 16 = x &&& 65535#32 := by
  sorry
...
theorem shl_or_lshr_comm_thm (x x_1 x_2 : _root_.BitVec 32) :
  (Option.bind (if 32 ≤ x.toNat then none else some (x_1 <<< x)) fun a =>
    ...
    Option.bind (if 32 ≤ x.toNat then none else some (x_1 <<< x)) fun a =>
      if 32 ≤ x.toNat then none else some ((a ||| x_2) >>> x) := by
  sorry
...
theorem mul_splat_fold_too_narrow_thm (x : _root_.BitVec 2) : 
    (x * 2#2) >>> 1 = x := by
  sorry
