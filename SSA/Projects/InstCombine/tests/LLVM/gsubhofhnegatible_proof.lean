...
theorem t2_thm (x x_1 : BitVec 8) :
  (Option.bind (if 8 ≤ x.toNat then none else some (214#8 <<< x.toNat)) fun a => some (x_1 - a)) ⊑
    Option.bind (if 8 ≤ x.toNat then none else some (42#8 <<< x.toNat)) fun a => some (a + x_1) := sorry
