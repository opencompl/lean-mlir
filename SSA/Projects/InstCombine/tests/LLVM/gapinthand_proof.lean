...
theorem test0_thm (x : BitVec 39) : x &&& 0#39 = 0#39 := sorry
...
theorem test8_thm (x : BitVec 999) : x &&& 0#999 = 0#999 := sorry
...
theorem test13_thm (x : BitVec 1024) :
  (Option.bind (if 1024 ≤ 1016 % 2 ^ 1024 then none else some (x.sshiftRight (1016 % 2 ^ 1024))) fun a =>
      some (a &&& 255#1024)) ⊑
    if 1024 ≤ 1016 % 2 ^ 1024 then none else some (x >>> (1016 % 2 ^ 1024)) := sorry
