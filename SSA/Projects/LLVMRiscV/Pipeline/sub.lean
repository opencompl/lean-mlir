...
def llvm_sub_lower_riscv_no_flag_self: LLVMPeepholeRewriteRefine 64 [Ty.llvm (.bitvec 64)] :=
  {lhs := llvm_sub_self , rhs := sub_riscv_self, correct := by
    ...
    simp_peephole
    simp only [LLVM.sub, Bool.false_eq_true, false_and, ↓reduceIte, LLVM.sub?, castriscvToLLVM,
      RTYPE_pure64_RISCV_SUB, castLLVMToriscv, BitVec.setWidth_eq, BitVec.sub_eq, sub_self,
      BitVec.ofNat_eq_ofNat, BitVec.signExtend_eq]
