...
def freeze_riscv_8 := [LV| {
  ^entry (%r1: i8):
    %toRISCV = "builtin.unrealized_conversion_cast" (%r1) : (i8) -> (!i64)
    %toLLVM = "builtin.unrealized_conversion_cast" (%toRISCV) : (!i64) -> (i8)
    llvm.return %toLLVM : i8
  }]

def llvm_freeze_lower_riscv_8 : LLVMPeepholeRewriteRefine 8 [Ty.llvm (.bitvec 8)] where
  lhs := freeze_llvm_8
  rhs := freeze_riscv_8
...
def freeze_riscv_16 := [LV| {
  ^entry (%r1: i16):
    %toRISCV = "builtin.unrealized_conversion_cast" (%r1) : (i16) -> (!i64)
    %toLLVM = "builtin.unrealized_conversion_cast" (%toRISCV) : (!i64) -> (i16)
    llvm.return %toLLVM : i16
  }]

def llvm_freeze_lower_riscv_16 : LLVMPeepholeRewriteRefine 16 [Ty.llvm (.bitvec 16)] where
  lhs := freeze_llvm_16
  rhs := freeze_riscv_16
...
def freeze_riscv_64 := [LV| {
  ^entry (%r1: i64):
    %toRISCV = "builtin.unrealized_conversion_cast" (%r1) : (i64) -> (!i64)
    %toLLVM = "builtin.unrealized_conversion_cast" (%toRISCV) : (!i64) -> (i64)
    llvm.return %toLLVM : i64
  }]
