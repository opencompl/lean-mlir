...
/-!
  This file implements the lowering for the `llvm.add` instructions for types i1, i8, i16 i32, i64.
-/
...
def llvm_add_lower_riscv_noflags_64 : LLVMPeepholeRewriteRefine 64 [Ty.llvm (.bitvec 64), Ty.llvm (.bitvec 64)] where
  lhs:= add_llvm_no_flags_64
  rhs:= add_riscv_64

def llvm_add_lower_riscv_nsw_flag_64 : LLVMPeepholeRewriteRefine 64 [Ty.llvm (.bitvec 64), Ty.llvm (.bitvec 64)] where
  lhs:= add_llvm_nsw_flags_64
  rhs:= add_riscv_64

def llvm_add_lower_riscv_nuw_flag_64 : LLVMPeepholeRewriteRefine 64 [Ty.llvm (.bitvec 64), Ty.llvm (.bitvec 64)] where
...
def add_match : List (Σ Γ, Σ ty, PeepholeRewrite LLVMPlusRiscV Γ ty) :=
  List.map (fun x => mkRewrite (LLVMToRiscvPeepholeRewriteRefine.toPeepholeUNSOUND x))
  [llvm_add_lower_riscv_noflags_64,llvm_add_lower_riscv_nsw_flag_64, llvm_add_lower_riscv_nuw_flag_64,
  llvm_add_lower_riscv_nuw_nsw_flag_64]
