...
XDSL_FUNC_ASM_DIR_PATH = (
    f"{ROOT_DIR_PATH}/SSA/Projects/LLVMRiscV/Evaluation/benchmarks/XDSL_FUNC_ASM/"
)
XDSL_FUNC_opt_ASM_DIR_PATH = (
    f"{ROOT_DIR_PATH}/SSA/Projects/LLVMRiscV/Evaluation/benchmarks/XDSL_FUNC_opt_ASM/"
)
...
def generate_benchmarks(num, jobs, llvm_opts):
    ...
    for filename in os.listdir(MLIR_single_DIR_PATH):
        ...
        idx += 1
        percentage = (
            (float(idx) + float(1)) / float(len(os.listdir(MLIR_single_DIR_PATH)))
        ) * 100
    ...
    for llvm_opt in llvm_opts:
        ...
        for filename in os.listdir(LLVMIR_DIR_PATH):
            ...
            idx += 1
            percentage = (
                (float(idx) + float(1)) / float(len(MLIR_translate_file2ret))
            ) * 100
    ...
    for filename in os.listdir(LLVM_DIR_PATH):
        ...
        idx += 1
        percentage = (
            (float(idx) + float(1)) / float(len(os.listdir(LLVM_DIR_PATH)))
        ) * 100
