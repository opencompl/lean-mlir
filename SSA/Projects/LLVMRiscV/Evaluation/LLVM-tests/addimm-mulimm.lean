...
/-!
  This file implements the `addimm-mulimm.ll` test case in the LLVM test suite:
  https://github.com/llvm/llvm-project/blob/b424207cdddfa2cbfc9129bbe0a31e47cb04e6dc/llvm/test/CodeGen/RISCV/addimm-mulimm.ll
-/

/-- ### add_mul_combine_accept -/

@[simp_denote]
def add_mul_combine_accept_i64_llvm := [LV| {
    ^entry (%x: i64):
...
@[simp_denote]
def add_mul_combine_accept_i64_riscv :=
  [LV| {
    ^entry (%x: i64):
...
def  add_mul_combine_accept_i64_pattern : LLVMPeepholeRewriteRefine 64 [Ty.llvm (.bitvec 64)] where
  lhs := add_mul_combine_accept_i64_llvm
  rhs := add_mul_combine_accept_i64_riscv
  correct := by
    simp_lowering
    simp
    bv_decide
