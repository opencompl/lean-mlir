  ...
  match env.find? declName with
    | some const =>
      try
        ...
        IO.println ("Error" ++ (toString e.getRef))
        return false
...
unsafe def analyzeTheoremsImpl (tac : TacticM Unit) : CommandElabM (Nat × Nat) := do
  ...
  for moduleName in modules do
    let (solved', unsolved') ← withImportModules #[
      ⟨ moduleName, false⟩, --,
      ⟨`SSA.Projects.InstCombine.TacticAuto, false⟩,
      ⟨`SSA.Projects.InstCombine.ForStd, false⟩,
      ⟨`SSA.Projects.InstCombine.ForMathlib, false⟩,
      ⟨`SSA.Projects.InstCombine.LLVM.Semantics, false⟩
      -- ⟨ `SSA.Experimental.Bits.Fast.Tactic , false⟩
      ] {} 0 <| fun (env : Environment) => do
      let res := works tac env moduleName
      Prod.fst <$> res.run'.toIO { fileName := "simple.lean", fileMap := default } { env := env }
...
elab "#analyze_theorems" : command => do
  let (solved, unsolved) ← analyzeTheorems (evalTactic (← `(tactic| (
  simp_alive_undef
  simp_alive_ops
  simp_alive_case_bash
  ensure_only_goal
