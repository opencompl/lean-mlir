...
def opToString (op : RISCV64.Op) : String :=
  ...
  | .remwu => "remwu"
  | .remu => "remu"
  | .addiw (_imm : BitVec 12) => s!"addiw \{imm = { _imm.toInt } : !riscv.reg }"
  | .lui (_imm : BitVec 20) => s!"lui \{imm = { _imm.toInt } : !riscv.reg }"
  ...
  | .divw => "divw"
  | .divwu => "diwu"
  | .addi (_imm : BitVec 12) => s!"addi \{imm = { _imm.toInt } : !riscv.reg }"
  | .slti (_imm : BitVec 12) => s!"slti \{imm = { _imm.toInt } : !riscv.reg }"
  | .sltiu (_imm : BitVec 12) => s!"sltiu \{imm = { _imm.toInt } : !riscv.reg }"
  | .andi (_imm : BitVec 12) => s!"andi \{imm = { _imm.toInt } : !riscv.reg }"
  | .ori (_imm : BitVec 12) => s!"ori \{imm = { _imm.toInt } : !riscv.reg }"
  | .xori (_imm : BitVec 12) => s!"xori \{imm = { _imm.toInt } : !riscv.reg }"
  | .czero.eqz => "czero.eqz"
  | .czero.nez => "czero.nez"
  | .sext.b => "sext.b"
  | .sext.h => "sext.h"
  ...
  | .binv => "binv"
  | .bset => "bset"
  | .bclri (_shamt : BitVec 6) => s!"bclri \{shamt = { _shamt.toInt } : !riscv.reg }"
  | .bexti (_shamt : BitVec 6) =>s!"bexti \{shamt = { _shamt.toInt } : !riscv.reg }"
  | .binvi (_shamt : BitVec 6) => s!"binvi \{shamt = { _shamt.toInt } : !riscv.reg }"
  | .bseti (_shamt : BitVec 6) => s!"bseti \{shamt = { _shamt.toInt } : !riscv.reg }"
  | .rolw => "rolw"
  | .rorw => "rorw"
  | .add.uw => "add.uw"
  | .sh1add.uw => "sh1add.uw"
...
instance : DialectDenote (RV64) where
  ...
  | .remw, regs, _ => REMW_pure64_signed_bv (regs.getN 1 (by simp [DialectSignature.sig, signature]))  (regs.getN 0 (by simp [DialectSignature.sig, signature]))
  | .remwu, regs, _ => REMW_pure64_unsigned_bv (regs.getN 1 (by simp [DialectSignature.sig, signature]))  (regs.getN 0 (by simp [DialectSignature.sig, signature]))
  | .rem, regs, _ => REM_pure64_signed_bv (regs.getN 1 (by simp [DialectSignature.sig, signature]))  (regs.getN 0 (by simp [DialectSignature.sig, signature]))
  | .remu, regs, _ => REM_pure64_unsigned_bv (regs.getN 1 (by simp [DialectSignature.sig, signature]))  (regs.getN 0 (by simp [DialectSignature.sig, signature]))
