...
def attributesToPrint: RISCV64.Op → String
  ...
  | .auipc (_imm : BitVec 20) => s!"\{immediate = { _imm.toInt} : si20 }" -- adding a s such that xdsl can parsee it, double-check when needed and when not
  | .slliw (_imm : BitVec 5) => s!"\{immediate = { _imm.toInt} : i5 }"
  | .srliw (_imm : BitVec 5) => s!"\{immediate = { _imm.toInt} : i5 }"
  | .sraiw (_imm : BitVec 5) => s!"\{immediate = { _imm.toInt} :i5 }"
  | .slli (_imm : BitVec 6) => s!"\{immediate = { _imm.toInt} : i6 }"
  | .srli (_imm : BitVec 6) => s!"\{immediate = { _imm.toInt} : i6 }"
  | .srai (_imm : BitVec 6) => s!"\{immediate = { _imm.toInt} : i6 }"
  ...
  | .bclri (_imm : BitVec 6) => s!"\{immediate = { _imm.toInt} : i6 }"
  | .bexti (_imm : BitVec 6) =>s!"\{immediate = { _imm.toInt} : i6 }"
  | .binvi (_imm : BitVec 6) => s!"\{immediate = { _imm.toInt} : i6 }"
  | .bseti (_imm : BitVec 6) => s!"\{immediate = { _imm.toInt} : i6 }"
  | RISCV64.Op.slli.uw (_imm : BitVec 6) => s!"\{immediate = { _imm.toInt} : i6 }"
  | .rori (_imm : BitVec 5) => s!"\{immediate = { _imm.toInt} : i5 }"
  | .roriw (_imm : BitVec 5) => s!"\{immediate = { _imm.toInt} : i5 }"
  | _ => ""
