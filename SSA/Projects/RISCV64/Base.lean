...
def attributesToPrint: RISCV64.Op → String
  ...
  | .slliw (imm : BitVec 5) => s!"\{immediate = { imm.toInt} : i5 }"
  | .srliw (imm : BitVec 5) => s!"\{immediate = { imm.toInt} : i5 }"
  | .sraiw (imm : BitVec 5) => s!"\{immediate = { imm.toInt} :i5 }"
  | .slli (imm : BitVec 6) => s!"\{immediate = { imm.toInt} : i6 }"
  | .srli (imm : BitVec 6) => s!"\{immediate = { imm.toInt} : i6 }"
  | .srai (imm : BitVec 6) => s!"\{immediate = { imm.toInt} : i6 }"
