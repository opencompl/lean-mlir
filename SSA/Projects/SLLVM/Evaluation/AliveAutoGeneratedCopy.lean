...
--     {h_elem : ∀ i (_ : i < as.length), A as[i] = A as[i]}
--     {xs : HVector A as}

-- -- @[simp, simp_denote]
-- -- theorem cast_rfl (i : Fin bs.length) :
-- --     (xs.cast rfl h_elem) = xs := by
-- --   sorry

-- simproc reduceGetCast ((HVector.cast _ _ _).get _) := fun e =>
--   let_expr HVector.get _α _A _as xs i := e
--     | return .continue
--   let_expr HVector.cast _α _β _A _B _as _bs h_len h_elem xs := xs
