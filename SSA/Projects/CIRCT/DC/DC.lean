...
def pack (x : ValueStream α) (y : TokenStream) : ValueStream α :=
  HandshakeStream.corec (β := ValueStream α × TokenStream) (x, y) fun ⟨x, y⟩ =>
    match x 0, y 0 with
...
def join (x y : TokenStream) : TokenStream  :=
  HandshakeStream.corec (β := TokenStream × TokenStream) (x, y) fun ⟨x, y⟩ =>
    match x 0, y 0 with
...
toType := fun
| Ty.tokenstream => DCOp.TokenStream
| Ty.tokenstream2 => DCOp.TokenStream × DCOp.TokenStream
| Ty.valuestream w => DCOp.ValueStream (BitVec w)
| Ty.valuestream2 w => DCOp.ValueStream (BitVec w) × DCOp.ValueStream (BitVec w)
| Ty.valuetokenstream w => DCOp.ValueStream (BitVec w) × DCOp.TokenStream
| Ty.variadicvaluetokenstream w => DCOp.VariadicValueStream w × DCOp.TokenStream
...
def_denote for DC where
  ...
  | .merge => fun s₁ s₂ => [DCOp.merge s₁ s₂]ₕ
  | .branch => fun s => [DCOp.branch s]ₕ
  | .fork => fun s => [DCOp.fork s]ₕ
  | .join => fun s₁ s₂ => [DCOp.join s₁ s₂]ₕ
  | .select => fun s₁ s₂ c => [DCOp.select s₁ s₂ c]ₕ
  | .sink => fun s => [DCOp.sink s]ₕ
  | .source => [DCOp.source]ₕ
  | .pack _ => fun s₁ s₂ => [DCOp.pack s₁ s₂]ₕ
  | .pack2 _ => fun s₁ => [DCOp.pack2 s₁]ₕ
  | .unpack _ => fun s => [DCOp.unpack s]ₕ
  | .unpack2 _ => fun s₁ s₂ => [DCOp.unpack2 s₁ s₂]ₕ
