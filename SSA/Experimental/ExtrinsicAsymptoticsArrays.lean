...
inductive Com.WF : Ctxt → Ty → Com → Prop
  ...
  | let : e.WF Γ ty → WF (Γ.push ty) ty' body → WF Γ ty' (.let ty e body)
...
def check : (c : Com) → Except String (Σ ty, PLift (c.WF Array.empty ty)) := go Array.empty
where
...
theorem check_ok : checkTy c ty → c.WF Array.empty ty := by
  unfold checkTy
  split
...
example : ex'.WF Array.empty .nat := check_ok (by decide)

-- ...is not more efficient than elaboration with the intrinsic representation
set_option profiler true
set_option maxRecDepth 10000
example : (mk_com' 50).WF Array.empty .nat := check_ok (by decide)
example : (mk_com' 100).WF Array.empty .nat := check_ok (by decide)

-- However, it becomes virtually instantaneous up to stack overflow-triggering sizes when using native reduction
example : (mk_com' 100).WF Array.empty .nat := check_ok (by native_decide)
example : (mk_com' 1000).WF Array.empty .nat := check_ok (by native_decide)
