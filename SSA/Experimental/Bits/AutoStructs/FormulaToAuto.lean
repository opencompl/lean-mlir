...
-- @[simp]
-- lemma BitVecs.cast_w {bvs : BitVecs n} (h : n' = n) : (h ▸ bvs).w = bvs.w := by
--   cases h; rfl

-- @[simp]
-- lemma BitVecs.cast_get (bvs : BitVecs n) (h : n' = n) (x : Fin n') : (h ▸ bvs).bvs.get x = (cast_w h) ▸ (bvs.bvs.get (x.cast h)) := by
--   cases h
--   rfl
