...
def Circuit.widthOne_sum (c : Circuit α) : Circuit (Width.n 1 ⊕ α) :=
  c.map inj
  where
  inj (a : α) : Width.n 1 ⊕ α := Sum.inr a
...
def CircuitProd.ofWidth1 : CircuitProd α (Width.n 1) :=
  fun (i : Width.n 1) => Circuit.ofBool
/--
bitwise AND term:
