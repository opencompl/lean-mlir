...
example (w : Nat) (a : BitVec w) : (w = 2) → ((a = - a) → a = 0#w) := by
  fail_if_success bv_automata_classic_nf
  sorry


example (w : Nat) (a : BitVec w) : (w = 1) → (a = 0#w ∨ a = 1#w) := by bv_automata_classic_nf
example (w : Nat) (a : BitVec w) : (w = 0) → (a = 0#w ∨ a = 1#w) := by bv_automata_classic_nf
example (w : Nat) : (w = 1) → (1#w + 1#w = 0#w) := by bv_automata_classic_nf
example (w : Nat) : (w = 0) → (1#w + 1#w = 0#w) := by bv_automata_classic_nf
example (w : Nat) : ((w = 0) ∨ (w = 1)) → (1#w + 1#w = 0#w) := by bv_automata_classic_nf
...
theorem sub_eq_mul_and_not_sub_xor (x y : BitVec w):
  ...
  all_goals bv_automata_classic_nf (config := {circuitSizeThreshold := 140 })
...
def alive_1 {w : ℕ} (x x_1 x_2 : BitVec w) : (x_2 &&& x_1 ^^^ x_1) + 1#w + x = x - (x_2 ||| ~~~x_1) := by
  bv_automata_classic_nf (config := { circuitSizeThreshold := 107 })
...
def test_OfNat_ofNat (x : BitVec 1) : 1#1 + x = x + 1#1 := by
  bv_automata_classic_nf -- can't decide things for fixed bitwidth.
          ...
          (decide
            (∀ (w : ℕ) (vars : List BitStream),
              (Predicate.binary BinaryPredicate.eq ((Term.var 0).or (Term.var 1))
                  ...
                  vars w =
                false))
          true SSA.Experimental.Bits.FastCopy.Tests._auxLemma.18))
...
example : ∀ (w : Nat) (x : BitVec w), x <<< (0 : Nat) = x := by intros; bv_automata_classic_nf
example : ∀ (w : Nat) (x : BitVec w), x <<< (1 : Nat) = x + x := by intros; bv_automata_classic_nf
example : ∀ (w : Nat) (x : BitVec w), x <<< (2 : Nat) = x + x + x + x := by
  intros w n
  -- rw [BitVec.ofNat_eq_ofNat (n := w) (i := 2)]
  intros; bv_automata_classic_nf
...
def test31 (w : Nat) (x : BitVec w) : x &&& x = x := by
  bv_automata_classic_nf (config := { stateSpaceSizeThreshold := 100 })
...
theorem add_eq_xor_add_mul_and (x y : BitVec w) :
    x + y = (x ^^^ y) + (x &&& y) + (x &&& y) := by
  bv_automata_classic_nf (config := { circuitSizeThreshold := 300 } )
...
theorem mul_five (x : BitVec w) : 5 * x = x + x + x + x + x := by
  bv_automata_classic_nf (config := { backend := .cadical })
...
theorem slow₁ (x : BitVec 32) :
  ...
  fail_if_success bv_automata_classic_nf (config := { circuitSizeThreshold := 30, stateSpaceSizeThreshold := 24 } )
  sorry
