...
\documentclass[onecolumn, sigconf]{acmart}
...
$ docker run -it siddudruid/oopsla25-width-indep
...
\subsubsection{Verifying the Results for the MBA dataset (\texttt{SSA/Experimental/Bits/Fast/Dataset2}).}
Please run:
\begin{script}
$ cat /code/lean-mlirSSA/Experimental/Bits/Fast/Dataset2/dataset2-cactus-plot-data.tex
\end{script}
This should produce the following output, where the timings might change
depending on the machine that is run on.
We expect the number of problems solved to be \emph{at least} 1500 for KInduction,
and all of \texttt{2500} for the other two solvers.
The absolute number of problems solved can vary by 5-10 problems for KInduction,
as the K-induction solver uses \texttt{cadical}, a SAT solver,
which can have different timings and problems solved depending on the machine available.
